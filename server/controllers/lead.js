...
export const createLead = async (event) => {
  try {
    ...
    // Save communication preferences if provided
    if (payload.contactMethod) {
      ...
      // attach helper field for immediate UI use
...
export const updateLead = async (event) => {
  try {
    ...
    const fields = ['alert','name','email','telephone','inquiryDate','leadSource','leadStatus','treatment','assigned','followUpDate','comments','softDeleted']
...
// CRM options (lead sources, treatments, statuses)
export const listOptions = async (event) => {
  try {
    ...
    // Seed defaults if empty for known categories
    if (!rows.length && ['lead_source','treatment','lead_status'].includes(category)) {
      ...
      const items = (defaults[category] || []).map((n, i) => typeof n === 'string' ? ({ organisationId: Number(orgId), category, name: n, ordering: i }) : ({ organisationId: Number(orgId), category, name: n.name, color: n.color, ordering: i }))
...
export const saveLeadCommunication = async (event) => {
  try {
    ...
    const body = await readBody(event)
    const payload = typeof body === 'string' ? JSON.parse(body) : body
    const { leadId, preferredContactMethod, preferredAppointmentDay, bestTimesToContact } = payload || {}
    if (!orgId) return error(401, 'Unauthenticated')
    if (!leadId) return error(400, 'leadId required')
...
export const addLeadNote = async (event) => {
  try {
    ...
    if (!orgId) return error(401, 'Unauthenticated')
    for (const k of ['leadId','title','date','time','channel','summary']) {
