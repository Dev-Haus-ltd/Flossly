...
const overlapWindowClause = (start, end, excludeAppointmentId = null) => {
  ...
  if (excludeAppointmentId)
    clause.id = { [Op.ne]: Number(excludeAppointmentId) };
  return clause;
};
...
const sanitizeChartMeta = (meta = {}) => {
  ...
  const links =
    meta.links && typeof meta.links === "object" && !Array.isArray(meta.links)
      ? meta.links
      : {};
  const favoriteCodeIds = Array.isArray(meta.favoriteCodeIds)
    ? meta.favoriteCodeIds
    : [];
  const softTissue =
    meta.softTissue &&
    typeof meta.softTissue === "object" &&
    !Array.isArray(meta.softTissue)
      ? meta.softTissue
      : {};
  const riskAssessment =
    meta.riskAssessment &&
    typeof meta.riskAssessment === "object" &&
    !Array.isArray(meta.riskAssessment)
      ? meta.riskAssessment
      : {};
  return {
    images,
  ...
  try {
    ...
    if (!_seededOrgs.has(orgIdNum)) {
      ...
      const missingDefaults = DEFAULT_ORGANISATION_TREATMENTS.filter((seed) => {
        const codeKey = String(seed.code || "")
          .trim()
          .toUpperCase();
        const nameKey = String(seed.name || "")
          .trim()
          .toLowerCase();
      ...
      if (missingDefaults.length) {
        await OrganisationTreatment.bulkCreate(
    ...
    const rows = await OrganisationTreatment.findAll({
      where: { organisationId: orgIdNum, active: true },
      order: [["name", "ASC"]],
    });
    return success(
      rows.map((t) => ({
...
export const createTreatment = async (event) => {
  try {
    const { orgId } = event.context.user;
    ...
    if (!name) {
      return error(400, "Treatment name is required");
    }

    // ❗ duplicate check (IMPORTANT)
    const existing = await OrganisationTreatment.findOne({
      where: {
    ...
    if (existing) {
      ...
      return success(updatedTreatment);
    }

    const newTreatment = await OrganisationTreatment.create({
      name,
      code:
      ...
      price: body.price ?? 0,
      defaultDuration: body.defaultDuration ?? 15,
      organisationId: Number(orgId),
      active: true,
      category: body.category ?? null,
      color: body.color ?? undefined,
    });
    return success(newTreatment);
  } catch (e) {
    return error(500, e.message || "Failed to create treatment");
  }
};

export const updateTreatment = async (event) => {
  try {
    const { orgId } = event.context.user;
    const body = await readBody(event);
    const treatmentId = Number(body.id);
    const name = String(body.name || "").trim();

    if (!treatmentId) {
      return error(400, "Treatment id is required");
    }
    if (!name) {
      return error(400, "Treatment name is required");
    }
    ...
    if (!treatment) {
      return error(404, "Treatment not found");
    }

    const duplicate = await OrganisationTreatment.findOne({
      where: {
        organisationId: Number(orgId),
        name,
        id: { [Op.ne]: treatmentId },
      },
    });
    if (duplicate) {
      return error(400, "A treatment with this name already exists");
    }
    ...
    const updatedTreatment = await treatment.update({
      ...
      code:
        body.code && String(body.code).trim()
          ? String(body.code).trim().toUpperCase().replace(/\s+/g, "_")
          : treatment.code || name.toUpperCase().replace(/\s+/g, "_"),
      price:
        body.price !== undefined && body.price !== null
          ? body.price
          : treatment.price,
      defaultDuration:
        body.defaultDuration !== undefined && body.defaultDuration !== null
          ? body.defaultDuration
          : treatment.defaultDuration,
      category:
        body.category !== undefined ? body.category : treatment.category,
      color: body.color !== undefined ? body.color : treatment.color,
      active: body.active !== undefined ? body.active : treatment.active,
    });

    return success(updatedTreatment);
  } catch (e) {
    return error(500, e.message || "Failed to update treatment");
  }
};

export const deleteTreatment = async (event) => {
  try {
    const { orgId } = event.context.user;
    const body = await readBody(event);
    const treatmentId = Number(body.id);
    ...
    await treatment.update({ active: false });
    return success({ id: treatment.id });
  } catch (e) {
    return error(500, e.message || "Failed to delete treatment");
  }
};
// --- Patients ---
export const listPatients = async (event) => {
  try {
    ...
    const rows = await DiaryPatient.findAll({
      where,
...
export const listPatientsPaged = async (event) => {
  try {
    ...
    const itemsPerPage = Math.max(
      1,
      Math.min(100, Number(q.itemsPerPage || q.perPage || 10)),
    );
    const search = (q.search || q.q || "").trim();
    const where = { organisationId: Number(orgId) };
  ...
  try {
    ...
    const data = {
      ...
      dentistRecallInterval:
        payload.dentistRecallInterval || payload.recallInterval || null,
...
export const updatePatient = async (event) => {
  try {
    ...
    const row = await DiaryPatient.findOne({
...
export const listAppointments = async (event) => {
  try {
    ...
    if (!dateStr && !patientId)
      return error(400, "date or patientId is required");
    const where = { organisationId: Number(orgId) };
    if (dateStr) {
...
export const createAppointment = async (event) => {
  try {
    ...
    if (!patientId && payload.patientName) {
      ...
      const p = await DiaryPatient.create({
        organisationId: Number(orgId),
        firstName,
        lastName: lastName || "-",
      });
    ...
    const incomingOrgTreatmentId = payload.treatmentId
      ? Number(payload.treatmentId)
      : null;
    if (incomingOrgTreatmentId) {
      const t = await OrganisationTreatment.findOne({
        where: { id: incomingOrgTreatmentId, organisationId: Number(orgId) },
      });
      if (t) {
...
export const getPatient = async (event) => {
  try {
    ...
    const p = await DiaryPatient.findOne({
      where: { id, organisationId: Number(orgId) },
    });
...
export const getPatientChart = async (event) => {
  try {
    ...
    return success({
      chart: row?.chartJson || {},
      meta: sanitizeChartMeta(row?.metaJson || {}),
    });
  } catch (e) {
    const msg =
      (e &&
...
export const savePatientChart = async (event) => {
  try {
    ...
    if (!existing) {
      ...
      return success({
        id: created.id,
        chart: created.chartJson,
        meta: sanitizeChartMeta(created.metaJson || {}),
      });
    }
    ...
    return success({
      id: existing.id,
      chart: existing.chartJson,
      meta: sanitizeChartMeta(existing.metaJson || {}),
    });
  } catch (e) {
    const msg =
      (e &&
...
export const savePatientChartTooth = async (event) => {
  try {
    ...
    if (
      !payload ||
      typeof payload.toothData !== "object" ||
      Array.isArray(payload.toothData)
    ) {
...
export const createTreatmentPlan = async (event) => {
  try {
    ...
    if (!patient) return error(404, "Patient not found");
    const planKey =
      String(payload?.planKey || "").trim() || `plan-${Date.now()}`;
    ...
    const count = await DiaryTreatmentPlan.count({
      where: { organisationId: Number(orgId), patientId },
    });
...
export const updateTreatmentPlan = async (event) => {
  try {
    ...
    if (payload.priority !== undefined)
      row.priority = Number(payload.priority || row.priority);
...
export const createTreatmentPlanItem = async (event) => {
  try {
    ...
    if (incomingPlanId && !resolvedPlanName) {
      const plan = await DiaryTreatmentPlan.findOne({
        where: {
          organisationId: Number(orgId),
          patientId,
          planKey: incomingPlanId,
        },
        attributes: ["name"],
      });
      resolvedPlanName = plan?.name || null;
    }
    const count = await DiaryTreatmentPlanItem.count({
      where: { organisationId: Number(orgId), patientId },
    });
    ...
    const practitionerName =
      payload?.practitionerName || payload?.clinicianName || null;
    ...
    const completedAt = payload?.completedAt
      ...
      : isCompleted
        ? new Date()
        : null;
    const completedByPractitionerId =
      parsePositiveIntOrNull(payload?.completedByPractitionerId) ||
      (isCompleted ? practitionerId : null);
    const completedByPractitionerName =
      payload?.completedByPractitionerName ||
      (isCompleted ? practitionerName : null);
...
export const updateTreatmentPlanItem = async (event) => {
  try {
    ...
    if (payload.appointmentGroupId !== undefined)
      row.appointmentGroupId = payload.appointmentGroupId || null;
    if (payload.priority !== undefined)
      row.priority = Number(payload.priority || row.priority);
    ...
    if (payload.practitionerId !== undefined)
      row.practitionerId = parsePositiveIntOrNull(payload.practitionerId);
    if (payload.completedAt !== undefined)
      row.completedAt = payload.completedAt
        ? new Date(payload.completedAt)
        : null;
    if (payload.completedByPractitionerId !== undefined) {
      row.completedByPractitionerId = parsePositiveIntOrNull(
        payload.completedByPractitionerId,
      );
    }
    if (payload.completedByPractitionerName !== undefined) {
      row.completedByPractitionerName =
        payload.completedByPractitionerName || null;
    }
    if (payload.conditionLabel !== undefined)
      row.conditionLabel = payload.conditionLabel || null;
    if (payload.treatmentId !== undefined)
      row.treatmentId = payload.treatmentId
        ? Number(payload.treatmentId)
        : null;
    if (payload.treatmentCode !== undefined)
      row.treatmentCode = payload.treatmentCode || null;
    if (payload.treatmentName !== undefined)
      row.treatmentName = payload.treatmentName || null;
    if (payload.treatmentCategory !== undefined)
      row.treatmentCategory = payload.treatmentCategory || null;
    if (payload.surface !== undefined) row.surface = payload.surface || null;
    if (payload.condition !== undefined)
      row.condition = payload.condition || null;
    if (payload.fdi !== undefined)
      row.fdi = payload.fdi ? Number(payload.fdi) : null;
    ...
    if (payload.paymentPlan !== undefined)
      row.paymentPlan = payload.paymentPlan || "private";
    if (payload.referrerId !== undefined)
      row.referrerId = parsePositiveIntOrNull(payload.referrerId);
    if (payload.referrerName !== undefined)
      row.referrerName = payload.referrerName || null;
    if (payload.invoiceDesc !== undefined)
      row.invoiceDesc = payload.invoiceDesc || null;
    if (payload.showOnInvoice !== undefined)
      row.showOnInvoice = payload.showOnInvoice !== false;
    const normalizedStatus = String(row.status || "").toLowerCase();
    if (normalizedStatus === "completed") {
      if (!row.completedAt) row.completedAt = new Date();
      if (!row.completedByPractitionerId && row.practitionerId)
        row.completedByPractitionerId = row.practitionerId;
      if (!row.completedByPractitionerName)
        row.completedByPractitionerName =
          row.practitionerName || row.clinicianName || null;
    } else if (
...
export const reorderTreatmentPlanItems = async (event) => {
  try {
    ...
    const orderedIds = Array.isArray(payload?.orderedIds)
      ? payload.orderedIds.map((v) => Number(v)).filter(Boolean)
      : [];
    ...
    const selectedRows = orderedIds
      ...
      .filter(Boolean);
    if (selectedRows.length !== orderedIds.length)
      return error(400, "orderedIds contains invalid items");

    let finalOrder = existing;
    if (appointmentIdRaw !== undefined || appointmentGroupId !== null) {
      const expectedAppointmentId = Number.isFinite(appointmentId)
        ? appointmentId
        : null;
      for (const row of selectedRows) {
        const byIdValid =
          appointmentIdRaw === undefined
            ? true
            : (row.appointmentId || null) === expectedAppointmentId;
        const byGroupValid =
          appointmentGroupId === null
            ? true
            : (row.appointmentGroupId || null) === appointmentGroupId;
        if (!byIdValid || !byGroupValid) {
          return error(
            400,
            "orderedIds must belong to the same appointment group",
          );
        }
      }
      ...
      if (orderedIds.length !== existing.length) {
        return error(
          400,
          "orderedIds must include all treatment plan items when appointmentId is not provided",
        );
      }
...
export const appointmentConflictCheck = async (event) => {
  try {
    ...
    const excludeAppointmentId = payload?.excludeAppointmentId
      ? Number(payload.excludeAppointmentId)
      : null;
...
export const bookFromTreatmentPlan = async (event) => {
  try {
    ...
    if (!treatmentItemIds.length)
      return error(400, "No treatment items selected for booking");
    const patient = await requirePatientInOrg(orgId, patientId);
    if (!patient) return error(404, "Patient not found");
    ...
    if (
      ...
      selectedItems.length !== treatmentItemIds.length
    ) {
      return error(400, "Selected treatment items are invalid");
    }
    const fallbackDentistId =
      selectedItems.find((item) => Number(item.practitionerId || 0) > 0)
        ?.practitionerId || null;
  ...
  try {
    ...
    if (payload?.date && payload?.time) {
      ...
      if (!start || Number.isNaN(start.getTime()))
        return error(400, "Invalid date/time");
      const currentStart =
        row.startTime instanceof Date ? row.startTime : new Date(row.startTime);
      const currentEnd =
    ...
    if (start && end && !isNaN(start.getTime()) && !isNaN(end.getTime())) {
      ...
      const dentistOverlap = await DiaryAppointment.count({
        where: { ...orgClause, ...notCancelled, ...overlapWindow, dentistId },
      });
      if (dentistOverlap > 0)
        return error(409, "Dentist already has an appointment at this time");
      if (patientId) {
        const patientOverlap = await DiaryAppointment.count({
          where: { ...orgClause, ...notCancelled, ...overlapWindow, patientId },
        });
        if (patientOverlap > 0)
          return error(409, "Patient already has an appointment at this time");
      }
...
export const getStats = async (event) => {
  try {
    ...
    if (USE_TZ_AGNOSTIC) {
      if (period === "day") {
        setClinicHours(start, 0, 0, 0, 0);
        setClinicHours(end, 23, 59, 59, 999);
      } else if (period === "week") {
        const day = baseDate.getUTCDay();
        const diff = (day + 6) % 7; // Monday-based
        start = new Date(baseDate);
        start.setUTCDate(baseDate.getUTCDate() - diff);
        setClinicHours(start, 0, 0, 0, 0);
        end = new Date(start);
    ...
    const rows = await DiaryAppointment.findAll({
      where: {
        organisationId: Number(orgId),
...
export const listNotes = async (event) => {
  try {
    ...
    const rows = await DiaryNote.findAll({
      where: { organisationId: Number(orgId), dentistId, date },
      order: [["createdAt", "DESC"]],
    });
    return success(rows);
  } catch (e) {
...
export const deleteNote = async (event) => {
  try {
    ...
    const id = Number(payload.id || 0);
    if (!id) return error(400, "id is required");
    const row = await DiaryNote.findOne({
      where: { id, organisationId: Number(orgId) },
    });
...
export const savePatientComfort = async (event) => {
  try {
    ...
    if (
      ...
      !["Detailed", "Minimal", "Visual"].includes(payload.communicationStyle)
    ) {
      return error(
        400,
        "Communication style must be one of: Detailed, Minimal, Visual",
      );
    }
    ...
    const comfortData = {
      ...
      anxietyLevel:
        payload.anxietyLevel !== undefined && payload.anxietyLevel !== null
          ? Number(payload.anxietyLevel)
          : null,
    ...
    const msg =
      (e &&
        (e.message ||
          (e.data && e.data.message) ||
          (e.original && (e.original.message || e.original.detail)))) ||
      "Internal server error";
    return error(500, msg);
  }
};
...
export const updatePatientComfort = async (event) => {
  try {
    ...
    if (!comfort) {
      return error(
        404,
        "Patient comfort record not found. Use save endpoint to create it first.",
      );
    }
    ...
    for (const field of updateFields) {
      if (payload[field] !== undefined) {
        if (field === "anxietyLevel") {
          comfort[field] =
            payload[field] !== null ? Number(payload[field]) : null;
        } else {
...
export const getPatientSurvey = async (event) => {
  try {
    ...
    try {
      ...
      console.warn(
        "DiaryPatientSurveys table not found, returning empty survey structure:",
        dbError.message,
      );
    }
...
export const savePatientForm = async (event) => {
  try {
    ...
    if (!formType || !["medical_history", "consent"].includes(formType)) {
      return error(
        400,
        'formType must be either "medical_history" or "consent"',
      );
    }
...
export const updatePatientForm = async (event) => {
  try {
    ...
    if (payload.patientComments !== undefined)
      updateData.patientComments = payload.patientComments || null;
    if (payload.ourComments !== undefined)
      updateData.ourComments = payload.ourComments || null;
    if (payload.additionalInfo !== undefined)
      updateData.additionalInfo = payload.additionalInfo || null;
