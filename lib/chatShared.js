...
export const resolveDmStatusIcon = (raw) => {
  ...
  if (status === "read") return "mdi-check-all";
  if (status === "delivered") return "mdi-check-all";
...
// Blue ticks for "read", grey for delivered/sent (WhatsApp convention)
export const resolveDmStatusColor = (raw) => {
  const status = String(raw || "").toLowerCase();
  if (status === "read") return "#0061FB";
  return "";
};
...
export const resolveWhatsAppStatusColor = (raw) => {
  const status = String(raw || "").trim().toLowerCase();
  if (status.includes("read")) return "#0061FB";
  return "";
};
