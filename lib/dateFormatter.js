...
export const formatDateOnly = (inputDate) => {
  ...
  export const formatDateDDMMYYYY = (inputDate) => {
    if (!inputDate) return '';
    // Strip time component from datetime strings — this is a date-only formatter and
    // parsing a UTC midnight datetime (e.g. "1990-05-15T00:00:00.000Z") in a UTC+
    // timezone would shift the displayed date back by one day.
    const normalized = typeof inputDate === 'string' && inputDate.length > 10
      ? inputDate.slice(0, 10)
      : inputDate;
    const date = toValidDate(normalized);
    if (!isValid(date)) return '';
    return formatDDMMYYYY(date);
  }
