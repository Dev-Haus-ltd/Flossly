...
export const downloadFile = (file) => {
  ...
  const baseUrl = buildAbsoluteLink(file?.link || "", config.public.BASE_URL)
  const sep = baseUrl.includes('?') ? '&' : '?'
  const fullUrl = `${baseUrl}${sep}v=${ver}`
...
export const formatCrmTriggerPreview = (trigger = {}) => {
  ...
  if (type === "inquiry_days") {
    ...
    if (days === 0) return "Immediately after enquiry";
    return crmDayLabel(days, "after enquiry");
  }
  if (type === "activation_days") {
    const days = Number(trigger?.days || 0);
    if (days === 0) return "Immediately when automation is activated";
    return crmDayLabel(days, "after activation");
  }
  ...
  if (type === "birthday_month_start") {
    const offset = Number(trigger?.offsetDays || 0);
    if (offset === 0) return "1st of birthday month";
    if (offset < 0) return crmDayLabel(offset, "before birthday month start");
    return crmDayLabel(offset, "after birthday month start");
  }
  ...
  if (type === "practice_anniversary") {
    const offset = Number(trigger?.offsetDays || 0);
    if (offset === 0) return "Practice anniversary";
    if (offset < 0) return crmDayLabel(offset, "before practice anniversary");
    return crmDayLabel(offset, "after practice anniversary");
  }
  if (type === "practice_anniversary_month_end") {
    const offset = Number(trigger?.offsetDays || 0);
    if (offset === 0) return "End of anniversary month";
    if (offset < 0) return crmDayLabel(offset, "before anniversary month end");
    return crmDayLabel(offset, "after anniversary month end");
  }
  if (type === "send_now") {
    return "Send immediately (next scheduled run)";
  }
  return "Set Trigger";
};
