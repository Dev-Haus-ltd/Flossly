...
export const mapWhatsAppLogToChatItem = (
  ...
  const hasAttachments = Array.isArray(row?.attachments)
  ...
  // Keep media-only rows so attachments can render without injecting fake placeholder text.
  if (!message && !hasAttachments && !MEDIA_TYPES.has(type)) return null;
