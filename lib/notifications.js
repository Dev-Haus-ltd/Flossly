...
export function buildNotificationUrl(notification) {
  ...
  if (notification.type === 'chatbot_message') {
    ...
    return conversationId ? `/reporting-bot?conversationId=${conversationId}` : '/reporting-bot';
  }

  const leadId = notification.data?.leadId || '';

  // External link takes priority (e.g. Calendly training booking)
  if (notification.data?.trainingUrl) return notification.data.trainingUrl;
