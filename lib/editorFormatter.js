  ...
  try {
    ...
    const pushNode = (node) => {
      ...
      if (node.nodeName === 'IMG') {
        ...
        if (url) blocks.push({ type: 'image', data: { file: { url }, caption: node.getAttribute('alt') || '', withBorder: false, withBackground: false, stretched: false } })
        return
      }
...
export function blocksToHtml(data) {
  ...
  return blocks.map((b) => {
    ...
    if (b.type === 'image') {
      ...
      if (!url) return ''
      const alt = (b.data?.caption || '').replace(/"/g, '&quot;')
      return `<img src="${url}" alt="${alt}" style="max-width:100%;height:auto;display:block;margin:8px auto;" />`
    }
    return ''
  }).join('')
}
