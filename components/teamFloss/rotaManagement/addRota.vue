  ...
  <div class="pa-5 bg-white">
    <div class="rota-section">
      ...
      <v-form ref="rotaForm" class="mt-4" @submit.prevent="submitForm">
        <!-- Select Practice - Hidden in Edit Mode -->
        ...
        <v-row v-if="!isEditMode" cols="2" class="d-flex flex-wrap mt-1">
          <v-col
        ...
        <!-- Rota Options - Hidden in Edit Mode -->
        <div v-if="!isEditMode">
        ...
        <!-- Create Rota block -->
        <div class="mt-2" v-if="isCreateRota || isEditMode">
          <v-row>
            <v-col cols="4">
              <!-- Title -->
              <h2 class="rota-title">{{ isEditMode ? 'Update rota' : 'Create a new rota' }}</h2>
              ...
              <TeamFlossRotaManagementEmployeeSelect
                            ...
                            <div class="mt-6 text-right">
                              <v-btn color="primary" variant="flat" type="submit">
                                {{ isEditMode ? 'Update Rota' : 'Create Rota' }}
                              </v-btn>
                            </div>
                          </v-col>
                        </v-row>
                ...
                if (isEditMode.value) {
                  try {
                    ...
                    if (res.code === 0) {
                      // Update rota users
                      try {
                        const rotaUsers = form.value.employees.map((el) => {
                          return { userId: el };
                        });
                        await rotaStore.addRotaUsers({ rotaId: form.value.id, users: rotaUsers });
                      } catch (userErr) {
                        console.error("Error updating rota users:", userErr);
                      }
                      ...
                      emit('onAddRota', res.data) // Reusing onAddRota to switch view
                    } else{
                      mainStore.setSnackbar({
                        type: "error",
              ...
              const isEditMode = computed(() => !!props.rotaToEdit);
              onMounted(async () => {
                if (isEditMode.value) {
                  form.value = { ...props.rotaToEdit };
                  
                  // Fetch the current users in the rota
                  try {
                    const usersRes = await rotaStore.getRotaUsers({ rotaId: props.rotaToEdit.id });
                    if (usersRes.code === 0 && usersRes.data) {
                      form.value.employees = usersRes.data.map(u => u.userId ?? u.user?.id).filter(id => !!id);
                    } else if (props.rotaToEdit.users) {
                      form.value.employees = props.rotaToEdit.users.map(u => u.userId);
                    }
                  } catch (err) {
                    // Fallback to users from props if API fails
                    if (props.rotaToEdit.users) {
                      form.value.employees = props.rotaToEdit.users.map(u => u.userId);
                    }
                  }
                  
                  // Get employees list for the organization
                  if (props.rotaToEdit.orgId) {
                    getUsers(props.rotaToEdit.orgId);
                  }
