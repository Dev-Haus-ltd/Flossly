<template>
  ...
  >
    <v-card-title class="title d-flex justify-space-between align-center py-3">
      <v-tabs v-model="activeMainTab" color="primary" class="custom-tabs">
        <v-tab value="forms">Forms</v-tab>
        <v-tab value="sent">Sent Consent Documents</v-tab>
      </v-tabs>
      <v-btn
        v-if="activeMainTab === 'forms' && !showFormView"
        color="primary"
        variant="flat"
        @click="openFormView"
      >
      ...
      <v-btn
        v-else-if="activeMainTab === 'forms' && showFormView"
        variant="outlined"
        color="primary"
        @click="closeFormView"
      >
    ...
    <v-card-text class="px-5 py-4">
          ...
          >
            <template #item.formType="{ item }">
              <v-chip
                size="small"
                :color="
                  item.formType === 'medical_history' ? 'primary' : 'secondary'
                "
              >
                {{
                  item.formType === "medical_history"
                    ? "Medical History"
                    : "Consent"
                }}
              </v-chip>
            ...
            <template #item.patientComments="{ item }">
              <span
                v-if="item.patientComments"
                class="text-truncate"
                style="max-width: 200px"
                :title="item.patientComments"
              >
            ...
            <template #item.ourComments="{ item }">
              <span
                v-if="item.ourComments"
                class="text-truncate"
                style="max-width: 200px"
                :title="item.ourComments"
              >
                {{ item.ourComments }}
              </span>
              <span v-else>-</span>
      ...
      <!-- Sent Consent Documents Tab Content -->
      <div v-else-if="activeMainTab === 'sent'">
        <ConsentDocumentManager :patient="props.patient" />
      </div>
    </v-card-text>
  </v-card>
</template>
...
import ConsentDocumentManager from "@/components/consent/ConsentDocumentManager.vue";

const props = defineProps({
  patient: { type: Object, default: null },
});
// console.log('patient in PatientForms.vue', props.patient)
const forms = ref([]);
...
const activeMainTab = ref("forms"); // 👈 New: Toggle between Forms and Sent Documents
...
const viewForm = (form) => {
  ...
  selectedForm.value =
    form.formType === "medical_history" ? "medical" : "consent";
...
const onFormSubmit = async (formData) => {
  try {
    ...
    const mainStore = useMainStore();
    mainStore.setSnackbar({
      title: error.message || "Failed to save form",
      type: "error",
    });
  }
};
